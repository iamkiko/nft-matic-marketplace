...
contract NFTMarket is ReentrancyGuard {
    ...
    struct MarketItem { // like an object/map
        uint itemId;
        address nftContract;
        uint256 tokenId;
    ...
    mapping(uint256 => MarketItem) private idToMarketItem; // fetch items by id
    ...
    function createMarketItem(address nftContract, uint256 tokenId, uint256 price) public payable nonReentrant { // nonReentrant is a modifier
    ...
    function fetchMarketItems() public view returns (MarketItem[] memory) {
        ...
        MarketItem[] memory items = new MarketItem[](unsoldItemCount); // creates it based off the 'type' declared earlier as MarketItem[]
        for (uint i = 0; i < itemCount; i++) {
            if (idToMarketItem[i + 1].owner == address(0)) {
                uint currentId = i + 1; // set to id that's currently being loops
